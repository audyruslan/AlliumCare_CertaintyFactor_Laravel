                          ...
                          <tr>
                            ...
                            <td class="align-middle" style="width: 30%">
                              @if ($setting_type_input->input_type == 'range')
                                <input class="w-100" type="range" id="rng-{{ $data->id }}" min="0" max="{{ $values->count() - 1 }}" value="{{ $data->value * ($values->count() - 1) }}">
                                <input type="hidden" id="id_role-{{ $data->id }}" name="id_role[]" value="{{ $data->id  }}">
                                <input type="hidden" id="in_val_evidence-{{ $data->id }}" name="evidance_value[]" value="{{ $data->value }}">
                                <div class="d-flex justify-content-between">
                                  <span >{{ $min->name }}</span>  
                                  <span id="rngOutput-{{ $data->id }}">{{ $data->value }}</span>
                                  <span >{{ $max->name }}</span>
                                </div>
                              @elseif ($setting_type_input->input_type == 'select')
                                ...
                                <select class="form-select" name="evidance_value[]" id="basicSelect">
                                  ...
                                  <option value="{{ $value->value }}" {{ $value->value == $data->value ? 'selected' : '' }} >{{ $value->name }}</option>
                                  @endforeach
                                </select>
                              @endif
                          </td>
...
<script>
    var rng_{{ $data1->id }} = document.getElementById("rng-{{ $data1->id }}");
    var ro_{{ $data1->id }} = document.getElementById("rngOutput-{{ $data1->id }}");
    var in_val_evidence_{{ $data1->id }} = document.getElementById("in_val_evidence-{{ $data1->id }}");
    var myRange_{{ $data1->id }} = [
        @foreach ($values as $value)
            {{ $value->value }},
        @endforeach
    ];
    function updateRange(){
        ro_{{ $data1->id }}.textContent = myRange_{{ $data1->id }}[parseInt(rng_{{ $data1->id }}.value, 10)].toFixed(2) * 100 + '%';
        in_val_evidence_{{ $data1->id }}.value = myRange_{{ $data1->id }}[parseInt(rng_{{ $data1->id }}.value, 10)].toFixed(2);
    };
    window.addEventListener("DOMContentLoaded", updateRange);
    rng_{{ $data1->id }}.addEventListener("input", updateRange);
</script>
@endforeach
@endsection